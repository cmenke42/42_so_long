...
/*   Updated: 2023/04/27 19:29:57 by cmenke           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "so_long_bonus.h"

//allocates a new double pointer and adds all line maps, one by one
static bool	ft_add_line(char *line_read, t_vars *vars)
{
	...
	if (!map_new)
		return (ft_error("Malloc error - add line **map_new", 1));
	y = -1;
...
bool	ft_read_map(char *map_name, t_vars *vars)
{
	...
	if (fd <= -1)
		return (ft_error("Error opening the map file", 1));
